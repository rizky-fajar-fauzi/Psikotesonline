const fs = require('fs');
const discQuestionsFile = fs.readFileSync('src/data/discQuestions.ts', 'utf8');

// The original JSON from the user
const userQuestions = [
  {
    "id": 1,
    "statements": [
...
let content = discQuestionsFile;
// Strip out "export const discQuestions: DISCQuestion[] = " and ";"
const jsonStr = content.substring(content.indexOf('['), content.lastIndexOf(']') + 1);

const originalData = JSON.parse(jsonStr);

const updatedData = originalData.map((q, idx) => {
  const newQ = { ...q };
  const userQ = userQuestions[idx];
  if (userQ) {
    newQ.options = newQ.options.map((opt, optIdx) => {
      return {
        ...opt,
        text: userQ.statements[optIdx]
      };
    });
  }
  return newQ;
...
const newContent = `import { DISCQuestion } from "../types";

export const discQuestions: DISCQuestion[] = ${JSON.stringify(updatedData, null, 2)};
`;

fs.writeFileSync('src/data/discQuestions.ts', newContent);
