...
// We want to add the Participant Email tool to the Admin Portal, right after or replacing the Resend Email tool.
// Actually, let's keep both or just change the "Resend Email Tool" to "Kirim Laporan ke Email Peserta" and call the /api/send-participant-email route.
              ...
              <div className="bg-indigo-50/70 border border-indigo-200 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-indigo-800">
                    Kirimkan salinan dokumen hasil evaluasi DISC ini ke email peserta/klien.
                  </p>
                </div>
...
code = code.replace(
  /{\/\* Resend Email Tool \*\/}[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/,
  resendToolStr.trim()
);
